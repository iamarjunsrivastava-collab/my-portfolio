...
const activities = [
  ...
  },
  {
    ...
    type: "Event Management", date: "2025",
    bullets: [
      ...
      {activities.map((act, i) => (
        <div key={i} className="work-item">
          <div className="flex items-center justify-between mb-1">
            <span style={{ fontSize: 11, fontWeight: 700, color: "#c9a227", textTransform: "uppercase", letterSpacing: "0.06em" }}>{act.type}</span>
            <span style={{ fontSize: 13, color: "var(--text-faint)" }}>{act.date}</span>
          </div>
          <p style={{ fontSize: 15, fontWeight: 700, color: "var(--text-main)", marginBottom: 2 }}>{act.title}</p>
          <div className="flex items-center gap-1 mb-3" style={{ fontSize: 13, color: "var(--text-muted)" }}>
            <MapPin size={12} color="#c9a227" />
            {act.org}
          </div>
          <ul style={{ paddingLeft: 0, listStyle: "none", margin: 0 }}>
            {act.bullets.map((b, j) => (
              <li key={j} className="flex items-start gap-2 mb-1" style={{ fontSize: 14, color: "var(--text-muted)" }}>
                <span style={{ marginTop: 7, width: 5, height: 5, borderRadius: "50%", background: "#c9a227", flexShrink: 0, display: "inline-block" }} />
                {b}
              </li>
            ))}
          </ul>
